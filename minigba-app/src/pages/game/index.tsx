...
export default function GamePage(){
  ...
  const refresh=useCallback(async()=>{const[local,localSaves,sessions,catalogItem]=await Promise.all([libraryRepository.get(romId),saveRepository.list(romId),playHistoryRepository.list(romId),romCatalogClient.find(romId).catch(()=>undefined)]);setGame(local);setSaves(localSaves);setHistory(sessions);setItem(catalogItem)},[romId])
  ...
  const play=()=>{if(!game)throw new Error('请先下载 ROM');return Taro.navigateTo({url:`/player/index?romId=${romId}`})}
  ...
  const clearHistory=async()=>{const confirm=await Taro.showModal({title:'清除此游戏的记录',content:'只清除会话明细，不删除累计时长、ROM 或存档。',confirmText:'清除',confirmColor:'#d43c62'});if(!confirm.confirm)return;await playHistoryRepository.clear(romId);await refresh()}
  ...
  return <View className='page-shell game-page'>
    ...
    <View className='detail-section rom-facts'><View className='detail-heading'><Text>ROM 信息</Text></View><Fact label='SHA-256' value={romId}/><Fact label='大小' value={item?formatBytes(item.sizeBytes):game?formatBytes(game.sizeBytes):'未知'}/><Fact label='地区 / 语言' value={`${item?.region??game?.region??'World'} / ${item?.language??game?.language??'未标注'}`}/><Fact label='分发许可' value={item?.license.name??game?.licenseName??'本地文件'}/>{game&&<Fact label='云存档' value={cloudLabel(game.cloudState)}/>}</View>
...
function Fact({label,value}:{label:string;value:string}){return <View className='fact-row'><Text>{label}</Text><Text className={label==='SHA-256'?'mono':''}>{value}</Text></View>}
