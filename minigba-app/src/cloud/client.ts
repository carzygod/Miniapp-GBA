...
export class CloudRequestError extends Error{constructor(readonly statusCode:number,message:string){super(message);this.name='CloudRequestError'}get terminal():boolean{return[400,401,403,404,413,422].includes(this.statusCode)}}
...
export class CloudClient{
  async login(deviceName:string):Promise<void>{
    ...
    const response=await Taro.request<{accessToken:string;userId:string}>({url:`${apiBase}/v1/auth/wechat/login`,method:'POST',data:{code:login.code,clientDeviceId,deviceName}})
  ...
  async logout():Promise<void>{try{await Taro.request({url:`${apiBase}/v1/auth/logout`,method:'POST',header:this.headers()})}finally{Taro.removeStorageSync(tokenKey);setSettingsAccountScope()}}
  ...
  async deleteRomSaves(romId:string):Promise<void>{const response=await Taro.request({url:`${apiBase}/v1/saves/${romId}`,method:'DELETE',header:this.headers()});if(response.statusCode!==204&&response.statusCode!==404)throw new CloudRequestError(response.statusCode,`删除游戏云数据失败 (${response.statusCode})`)}
