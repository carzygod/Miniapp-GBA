...
describe('account-scoped settings',()=>{
  ...
  it('never accepts a malformed account scope',()=>{storage.set('minigba.settings.activeScope.v1','../../other-user');expect(activeScope()).toBe('anonymous');setSettingsAccountScope('------------------------------------');expect(activeScope()).toBe('anonymous')})
})
