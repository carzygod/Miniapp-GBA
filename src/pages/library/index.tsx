...
export default function LibraryPage(){
  ...
  const[busyRomId,setBusyRomId]=useState('')
  ...
  const localById=useMemo(()=>new Map(games.map(game=>[game.romId,game])),[games])
  ...
  const visibleCatalog=useMemo(()=>catalog.filter(item=>(genre==='全部分类'||item.genres.includes(genre))&&`${item.title} ${item.gameCode??''} ${item.genres.join(' ')}`.toLowerCase().includes(normalized)).sort((a,b)=>Number(b.featured)-Number(a.featured)||a.title.localeCompare(b.title,'zh-CN')),[catalog,genre,normalized])
  ...
  const install=async(item:RomCatalogItem)=>{
    if(localById.has(item.romId)){await openGame(item.romId);return}
    setBusyRomId(item.romId);setDownloadProgress(0)
    try{await libraryRepository.importCatalogItem(item,setDownloadProgress);await refreshLocal();Taro.showToast({title:'已加入游戏库',icon:'success'});await openGame(item.romId)}catch(error){showError(error)}finally{setBusyRomId('');setDownloadProgress(0)}
  }
  ...
  return <View className='page-shell home-page'>
    ...
    {mode==='square'&&<View className='shelf-section'><View className='section-heading'><View><Text className='section-title'>ROM 广场</Text><Text className='section-count'>{catalogStatus==='stale'?'缓存目录':`${visibleCatalog.length} 个授权条目`}</Text></View><Button className='refresh-button' loading={catalogStatus==='loading'} onClick={()=>refreshCatalog(true)}>刷新</Button></View>{catalogStatus==='error'?<View className='catalog-state'><Text>{catalogError}</Text><Button onClick={()=>refreshCatalog(true)}>重试</Button></View>:catalogStatus==='loading'&&!catalog.length?<View className='catalog-state'>正在读取 ROM 目录</View>:!visibleCatalog.length?<View className='catalog-state'>没有匹配的 ROM</View>:<View className='catalog-shelf'>{visibleCatalog.map((item,index)=>{const installed=localById.get(item.romId),downloading=busyRomId===item.romId;return <View className='catalog-row' key={item.romId} onClick={()=>openGame(item.romId)}><GameArtwork coverUrl={item.coverUrl} title={item.title} tone={index}/><View className='catalog-copy'><View className='catalog-title-line'><Text className='catalog-title'>{item.title}</Text>{item.featured&&<Text className='featured-label'>精选</Text>}</View><Text className='catalog-meta mono'>{item.gameCode||'HOMEBREW'} · {formatBytes(item.sizeBytes)} · {item.region||'World'}</Text><Text className='catalog-description'>{item.description||item.genres.join(' · ')||'授权发布'}</Text><View className='catalog-tags'><Text>{item.license.name}</Text>{item.genres.slice(0,2).map(value=><Text key={value}>{value}</Text>)}</View></View><Button className={`shelf-action ${installed?'installed':''}`} loading={downloading} onClick={event=>{event.stopPropagation();install(item).catch(showError)}}>{downloading?`${downloadProgress}%`:installed?'详情':'获取'}</Button></View>})}</View>}</View>}
