...
- 16 个测试文件、62 个测试通过；语句覆盖率 `63.79%`、分支 `58.38%`、函数 `66.15%`、行 `75.68%`。R2 schema v2 测试覆盖相对 URL、host allowlist、重复 catalog ID/object key、URL 与 object key 一致性、可选许可、无预置 digest、缓存回退，以及远端关闭时 981 项内置目录零请求启动；本地内容 ID、输入、ZIP、存档、云同步、诊断和游玩计时测试继续通过。
...
- App 只有在编译期 HTTPS API 地址已配置且本地会话有效时才刷新云会话；刷新成功后才处理该 UUID 专属的持久化同步队列。API 地址为空、匿名态、残留 token 或刷新失败均不会触发 `/v1/*` 请求，401 会清除失效 token，账户切换不会消费其他账户的待同步任务。
...
- 当前三个独立仓库 HEAD 分别为 Core `5045490add4e9691d1c005aeb84c9886d2489536`、App `b04fc700d7b84c9116eb4507a3579fd3107ca76e`、API `962e0bbbe26a82d1001ba62fb68e5bc468d1e859`，工作树均为空且 `git fsck` 通过。
