...
App 候选构建必须在 `project.config.json` 中固定已验证的微信基础库版本，并校验该字段进入 `dist/project.config.json`。本候选固定为本机完整缓存的 `3.15.2`；不得依赖开发者工具自动选择最新或灰度基础库，因为不完整的基础库缓存会在业务代码执行前表现为模拟器 HTTP 500 或 `WAServiceMainContext timeout`。
