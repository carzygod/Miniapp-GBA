...
项目只处理用户自行提供或已获得合法授权的 ROM。仓库不包含、搜索、销售或分发商业 ROM、官方 GBA BIOS，也不会把 ROM 上传到云端。

## 当前状态

- 微信小程序已实现 ROM 导入与隔离、完整虚拟按键、Canvas 播放器、本地电池存档、即时状态存档、截图、快进、音频和诊断页面。
- 云端已实现微信身份交换、会话刷新、版本历史、乐观并发、冲突副本、删除与账户清除流程。
- App 的 TypeScript、ESLint、45 个 Vitest 测试及 Taro production build 已通过。
...
正式构建还需要注入 HTTPS API origin 和授权 ROM 下载 host：
...
从 [文档中心](./docs/README.md) 开始阅读。产品需求、技术设计、核心移植、存储同步、测试验收、安全合规、交付计划和当前阻断均在 `docs/` 中维护。
