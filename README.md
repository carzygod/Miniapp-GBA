...
项目只处理用户自行提供或运营方已取得明确分发授权的 ROM。仓库不包含、销售或分发未授权商业 ROM、官方 GBA BIOS，也不会把用户私有 ROM 上传到云端。首页 ROM 广场读取 Cloudflare R2 上经过逐项权利审核的只读 manifest。
...
- App 的 TypeScript、ESLint、55 个 Vitest 测试、R2 manifest 校验及 Taro production build 已通过。
...
- R2 只发布运营方授权目录和对象；客户端不包含 R2/S3 写凭证，也不列举任意桶对象。
